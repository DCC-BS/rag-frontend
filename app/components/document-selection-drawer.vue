<template>
    <ClientOnly>
        <UDrawer v-model:open="isOpen" direction="left" :title="t('chat.selectDocuments')"
            ...
            <!-- Trigger button -->
            <UButton color="neutral" variant="outline" icon="i-heroicons-document-text" :label="buttonLabel"
                size="sm" />
                ...
                <div class="flex h-full min-w-96 max-w-md flex-col">
                        ...
                        <div class="mb-3 flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-gray-600 text-sm dark:text-gray-400">
                                    {{ t('chat.selectDocumentsDescription', {
                    ...
                    <div class="flex-1 overflow-y-auto p-4">
                        ...
                        <div class="py-12 text-center" v-else-if="documents?.documents?.length === 0">
                            ...
                            <UButton v-if="searchQuery" :label="t('documents.clearSearch')" color="primary"
                                variant="solid" @click="clearSearch" />
                        </div>
                        ...
                        <div v-else-if="documents && documents.documents?.length > 0" class="space-y-4">
                            ...
                            <div class="space-y-3">
                                <DocumentSelectionItem v-for="document in paginatedDocuments" :key="document.id"
                                    ...
                                    :can-select="canSelectMore || isDocumentSelected(document.id)"
                                    @select="selectDocument" @deselect="deselectDocument" />
                            </div>

                            <!-- Pagination -->
                            <div v-if="totalPages > 1" class="flex justify-center mt-4">
                                <UPagination v-model:page="currentPage" :total="totalDocuments"
                                    :items-per-page="itemsPerPage" show-edges :sibling-count="1" size="sm"
                                    color="primary" variant="outline" active-color="primary" active-variant="solid" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-gray-200 border-t p-4 dark:border-gray-700">
                        <div class="flex gap-2">
                            <UButton color="neutral" variant="outline" :label="t('common.close')"
                                @click="isOpen = false" class="flex-1" />
                            <UButton color="primary" variant="solid" :label="t('chat.chatWithSelected')"
                                :disabled="selectedDocuments.length === 0" @click="handleChatWithSelected"
                                class="flex-1" />
                        </div>
                    </div>
                </div>
