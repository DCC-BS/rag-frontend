...
const { fetchDocument, loading: viewerLoading, error: viewerError } = useDocumentViewer();
...
const findUserDocument = async (fileName: string): Promise<UserDocument | undefined> => {
...
async function handleDocumentClick(): Promise<void> {
  ...
  try {
    ...
    const result = await fetchDocument(userDocument.id, userDocument.file_name);
    ...
    if (result) {
      ...
      toast.add({
        title: t("documents.failedToLoad"),
        description: viewerError.value || t("documents.unableToLoad", {
          fileName: fileName,
        }),
        icon: "i-heroicons-exclamation-triangle",
        color: "error",
